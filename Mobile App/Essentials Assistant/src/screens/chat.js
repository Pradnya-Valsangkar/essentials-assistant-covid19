...
import { StyleSheet, KeyboardAvoidingView, Platform, ScrollView, View, Text, TextInput, Button, TouchableOpacity, Linking } from 'react-native';
...
const Chat = function (props) {
  console.log("Type of chatbot ",props)
  ...
  const MapLink = (props) => {
    ...
    return (
      <TouchableOpacity onPress={() => { props.navigation.navigate('Map', { item: props }); }}>
  ...
  const getSession = () => {
    asstType = props.route.params.type ||'general'
    return session(asstType)
      .then(sid => {
        setSessionId(sid);
        return sid;
      });
  ...
  const sendMessage = () => {
    console.log("Payload chat bot type ",props.route.params.type)
    const payload = {
      type: props.route.params.type || "general",
      text: input.trim(),
      sessionid: sessionId
    };
