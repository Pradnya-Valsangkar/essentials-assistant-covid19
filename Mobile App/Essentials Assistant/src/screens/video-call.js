...
import { Text, TouchableOpacity, View, YellowBox, StyleSheet }                                          from 'react-native';
...
/* ==============================
 Global variables
 ================================ */
//const url =   'https://1b30425be4b0.ngrok.io';

let url = Config.VIDEO_CALL_SOCKET_URL;
...
class Videocall extends Component {
  ...
  componentDidMount() {
    appClass = this;
    const appointmentDetails = this.props.appointment
    console.log("appointment details in video call", appointmentDetails)
    this.setState({roomID:appointmentDetails.id})
    getLocalStream();
  }
  ...
  handleEndCall = () => {
    socket.disconnect();
    // socket.emit('disconnect',callBackDisconnect);
    console.log("emit disconnect")
    this.props.vc.resetVideoCallData();
  };
  ...
  render() {
    ...
    return (
      ...
      <View style={{marginTop:50}}>
        <TouchableOpacity onPress= {this.handleEndCall}>
            <Text style={styles.button}>End Video Call</Text>
        </TouchableOpacity>
      </View>
      </View>
    );
